...
describe("loadBestAvailableAdapter", () => {
  ...
  it("uses the wasm adapter when the generated module exports the expected API", async () => {
    const loaded = await loadBestAvailableAdapter(async () => ({
      ...
      airway_spatial_tile_keys: async () => "[]",
      flight_plan_insert_anchor: async () => "{\"Airport\":\"KAAA\"}",
      materialize_airway_selection_from_branches: async () => "{\"selection\":{\"airway_name\":\"V2\",\"branch_key\":\"A\",\"entry\":{\"airway_name\":\"V2\",\"branch_key\":\"A\",\"branch_point_index\":0,\"sequence\":0,\"nav_ref\":{\"Fix\":\"A\"},\"distance_from_anchor_nm\":0,\"previous_nav_ref\":null,\"next_nav_ref\":null},\"exit\":{\"airway_name\":\"V2\",\"branch_key\":\"A\",\"branch_point_index\":1,\"sequence\":1,\"nav_ref\":{\"Fix\":\"B\"},\"leg_offset_from_entry\":1,\"is_entry\":false,\"distance_from_target_nm\":0},\"origin_distance_nm\":0,\"destination_distance_nm\":0,\"total_anchor_distance_nm\":0},\"airway\":{\"name\":\"V2\",\"branch_key\":\"A\",\"entry\":{\"Fix\":\"A\"},\"exit\":{\"Fix\":\"B\"}},\"resolvedLegs\":[]}",
      project_flight_plan_route_from_positions: async () => "[]",
      suggest_airways_near_from_points: async () => "[]",
      suggest_waypoint_identifiers_from_candidates: async () => "[]",
    }));
