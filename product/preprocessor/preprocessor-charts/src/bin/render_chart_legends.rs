...
use preprocessor_charts::build_family_legends;
...
fn main() -> anyhow::Result<()> {
    ...
    let family = match args.next().as_deref() {
        ...
        None => bail!("usage: render_chart_legends <sec|tac|enr-l|enr-h> <chart-work-dir>"),
    };
    let work_dir = PathBuf::from(args.next().context("missing chart work directory")?);
    if args.next().is_some() {
        bail!("unexpected extra argument");
    }
    let result = build_family_legends(family, &work_dir)?;
