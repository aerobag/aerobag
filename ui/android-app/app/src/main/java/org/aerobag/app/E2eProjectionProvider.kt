...
internal object E2eProjectionRegistry {
    ...
    fun publish(resourceId: String, state: String, owner: Any) {
        entries[resourceId] = Entry(
            owner = owner,
            snapshot = E2eProjectionSnapshot(state, revision.incrementAndGet()),
        )
    }
...
class E2eProjectionProvider : ContentProvider() {
    ...
    override fun query(
        ...
        if (viewId !in E2eProjectionRegistry.KnownViewIds) return null
    ...
    private companion object {
        val Columns = arrayOf("resource_id", "state", "revision", "present")
    }
}
