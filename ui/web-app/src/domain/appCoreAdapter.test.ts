...
describe("loadBestAvailableAdapter", () => {
  ...
  it("uses the wasm adapter when the generated module exports the expected API", async () => {
    const loaded = await loadBestAvailableAdapter(async () => ({
      ...
      tick_debug_ownship_driver_in_session_paged: async () => JSON.stringify({ state: "complete", result: JSON.parse(snapshotJson) }),
      ...
      get_map_overlay_in_session: async () => "{\"state\":\"complete\",\"result\":{\"visible_features\":[],\"visible_metars\":[],\"visible_pireps\":[],\"airspace_paths\":[],\"tfr_paths\":[],\"airspace_labels\":[],\"warnings\":[]}}",
      ...
      get_nexrad_overlay_in_session: async () => "{\"state\":\"complete\",\"result\":{\"status\":{\"state\":\"hidden\"},\"frames\":[]}}",
      ...
      nexrad_frame_bytes_in_session: async () => new Uint8Array(),
