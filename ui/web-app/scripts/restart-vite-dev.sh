...
LIVE_FEEDS_ORIGIN_OVERRIDE="${AEROBAG_LIVE_FEEDS_ORIGIN:-}"
ONLY_TEAR_DOWN=0
...
DEFAULT_LIVE_FEEDS_ORIGIN=""
LIVE_FEEDS_ORIGIN="${LIVE_FEEDS_ORIGIN_OVERRIDE:-${AEROBAG_LIVE_FEEDS_ORIGIN:-}}"
...
CLOUD_SERVER_BASE_URL="${AEROBAG_CLOUD_SERVER_BASE_URL:-http://aerobag-dev.iac.jonh.net:18080/cloud/}"

exec env \
  AEROBAG_REPO_ROOT="$REPO_ROOT" \
  AEROBAG_UI_TARGET_ROOT="$UI_TARGET_ROOT" \
  AEROBAG_ARTIFACT_READ_PATH="$ARTIFACT_READ_ROOT" \
  AEROBAG_CLOUD_SERVER_BASE_URL="$CLOUD_SERVER_BASE_URL" \
  AEROBAG_SERVICE_BULLETIN_URLS="${AEROBAG_SERVICE_BULLETIN_URLS:-http://aerobag-dev.iac.jonh.net:18080/service/bulletins-v1.json}" \
  AEROBAG_E2E_ENABLED="${AEROBAG_E2E_ENABLED:-1}" \
  "${LIVE_FEEDS_ENV[@]}" \
