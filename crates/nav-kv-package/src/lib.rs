...
pub fn xz_compress_bytes(bytes: &[u8]) -> Result<Vec<u8>, String> {
    let mut input = Cursor::new(bytes);
    let mut output = Vec::new();
    lzma_rs::xz_compress(&mut input, &mut output)
        .map_err(|err| format!("failed to xz-compress nav-kv page: {err}"))?;
    Ok(output)
}
...
pub fn write_stored_xz_package_bytes(
    manifest: &[u8],
    root: &[u8],
    pages: &[Vec<u8>],
    ...
    for (index, page) in pages.iter().enumerate() {
        let encoded = xz_compress_bytes(page)?;
...
mod tests {
    ...
    #[test]
    fn package_round_trips_xz_pages_in_stored_zip() {
        let manifest = br#"{"page_count":2}"#;
        let root = b"root";
        let pages = vec![b"page-one".to_vec(), b"page-two".to_vec()];
        let package = write_stored_xz_package_bytes(manifest, root, &pages).unwrap();
