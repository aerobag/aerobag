...
fn sync_live_feed_overlay_status_records(session: &mut UiSession) -> Vec<UiInvalidation> {
    ...
    let metars_visible = session.map.layer_state().metars.visible;
...
fn sync_terrain_status_record(
    ...
    let changed = if session.map.layer_state().terrain_warning.visible {
...
fn sync_nexrad_status_record(
    ...
    let nexrad_visible = session.map.layer_state().nexrad.visible;
...
pub fn select_map_family_in_session(
    ...
    let mut session_guard = slot.lock_running()?;
    let session = &mut *session_guard;
    if session.map.raster_catalog().is_none() {
        let catalog = match crate::had_ops::raster_map_catalog_from_nav_kv(
            session_nav_kv_store(session)?,
            None,
            Some(family_id),
    ...
    let Some(catalog) = session.map.raster_catalog_mut() else {
        return Err(AppError {
            kind: AppErrorKind::Internal,
            message: "session raster map catalog disappeared during selection".to_string(),
        });
    };
...
fn install_guidance_leg_geometry(
    ...
    session
        .flight_plan
        .install_guidance_leg_geometry(geometries);
...
fn sync_guidance_geometry_for_session(
    ...
    let Some(plan) = session.flight_plan.active_plan().cloned() else {
        session.flight_plan.clear_guidance_leg_geometry();
        crate::core_debug_log(
            "plan.guidance.sync.core_phase",
            &serde_json::json!({
                "phase": "empty_plan",
...
pub fn project_flight_plan_route_in_session(handle: u32) -> AppResult<HadOperationOutcome> {
    ...
    let mut session_guard = slot.lock_running()?;
    let session = &mut *session_guard;
    let store = session_nav_kv_store(session)?.clone();
    match session
        .flight_plan
        .project_route(&store, session.nav_data.generation())
    {
        Ok((projection, _rebuilt)) => Ok(HadOperationOutcome::complete(
            serde_json::to_value(projection).map_err(|err| AppError {
                kind: AppErrorKind::Internal,
                message: err.to_string(),
            })?,
        )),
...
fn planner_atmosphere_for_selection(
    selection: AltitudePlannerWindSelection,
    weather: &WeatherController,
) -> crate::had_ops::PlannerAtmosphereSelection<'_> {
    match selection {
...
pub fn advance_nav_kv_store_in_session_with_open_result(
    ...
    let selected_map_id = live
        ...
        .raster_catalog()
        .map(|catalog| catalog.selected_map_id.clone());
    let selected_family_id = live.map.raster_catalog().and_then(|catalog| {
        catalog
            .family_options
            .iter()
    ...
    let rebuild = (|| -> Result<UiSessionSnapshot, SessionSnapshotProjectionError> {
        ...
        candidate
            .map
            .replace_raster_catalog(Some(crate::had_ops::raster_map_catalog_from_nav_kv(
                store,
                selected_map_id.as_deref(),
...
pub fn ingest_point_tiles_in_session(handle: u32, tiles: &[PointTilePayload]) -> AppResult<()> {
    ...
    let mut session_guard = slot.lock_running()?;
    let session = &mut *session_guard;
    let runtime = session.map.runtime_mut();
    for tile in tiles {
...
pub fn report_session_resource_failure_in_session_at_epoch_ms(
    ...
    if LiveFeedsState::handles_resource(resource_id) {
        ...
        && session.map.layer_state().terrain_warning.visible
    {
        upsert_data_status_record(
            session,
        ...
        && crate::adsb::AdsbSessionState::handles_resource(resource_id)
    {
        ...
        if ownship_resource {
            ...
            sync_adsb_ownship_status_record(session);
        } else if session.map.layer_state().traffic.visible {
            upsert_data_status_record(
                session,
                DataStatusRecord::new(
                    ADSB_TRAFFIC_STATUS_ID,
...
fn install_live_feed_payloads(session: &mut UiSession) -> AppResult<()> {
    if session.weather.live_feeds().current_loaded() {
            ...
            .has_product_current_version("obstacles")
        {
            ...
            session.weather.runtime_mut().obstacle_tile_cache.clear();
            session.map.set_obstacle_layer(None);
            clear_data_status_record(session, LIVE_FEED_OBSTACLES_STATUS_ID);
        }
        if !session
            .weather
...
fn install_live_obstacle_had_with_parsed_manifest(
    ...
    session.map.set_obstacle_layer(Some(obstacle_layer));
...
fn metar_importance_required_for_surface(session: &UiSession, metrics: &MapSurfaceMetrics) -> bool {
    ...
    let Some(layer) = session.map.overlay_config().metar_layer.as_ref() else {
        return false;
    };
    crate::overlay_surface_decision(*metrics, session.map.overlay_config()).metar_tile_zoom
        == Some(layer.min_zoom)
}
...
fn ensure_vector_manifest_loaded(session: &mut UiSession) -> Result<(), HadReadError> {
    if session.map.vector_manifest_loaded() {
        return Ok(());
    }
    let manifest_json = {
        let store = session.nav_data.store().ok_or_else(|| {
...
fn ensure_vector_inputs_loaded(
    ...
    crate::core_perf_debug_log("map.overlay.vector_inputs", || {
        serde_json::json!({
            ...
            "cached_vector_tiles": session.map.runtime().vector_tile_cache.len(),
            "cached_airspace_features": session.map.runtime().airspace_feature_cache.len(),
        })
    });
    result
}
...
fn ensure_vector_inputs_loaded_impl(
    ...
    for _ in 0..8 {
        ...
        for tile in vector_tiles {
            runtime.vector_tile_cache.insert(
...
fn ensure_live_obstacle_inputs_loaded(
    ...
    let Some(layer) = session.map.overlay_config().obstacle_layer.clone() else {
        return Vec::new();
    };
...
pub fn get_map_overlay_in_session_with_point_display_scale_at_epoch_ms(
    ...
    let offline_region_records = if session.map.layer_state().offline_regions.visible {
        let store = session.nav_data.store().ok_or_else(|| AppError {
            kind: AppErrorKind::Internal,
...
pub fn get_scheduled_terrain_overlay_in_session_at_epoch_ms(
    ...
    if !session.map.layer_state().terrain_warning.visible {
...
fn session_nav_kv_store(session: &UiSession) -> AppResult<&NavKvStore> {
    session.nav_data.store().ok_or_else(|| AppError {
        kind: AppErrorKind::Internal,
...
fn project_session_app_ui_state(
    ...
    if flight_plan_projection.rebuilt {
        session
            .diagnostics
            .flight_plan_projection_count
            .fetch_add(1, Ordering::Relaxed);
...
impl SessionSituationSourceHandler for PlanPreviewSituationSourceHandler {
    ...
    fn input_disabled_reason(
        ...
        let records = session
            .flight_plan
            .active_plan()
            .map(|plan| plan_preview_legs(plan, session.flight_plan.guidance_leg_geometry()))
            .unwrap_or_default();
        if records.is_empty() {
...
fn project_active_leg_nav_element(
    ...
    let materialized = crate::flight_plan_materialization::MaterializedFlightPlan::build(
        plan,
        session.flight_plan.guidance_leg_geometry(),
        session.situation.ownship().render.position,
    )
    .map_err(|err| HadReadError::Fatal(err.message))?;
...
fn sync_plan_preview_to_active_leg(session: &mut UiSession) -> AppResult<()> {
    ...
    let leg_index = plan
        ...
        .unwrap_or(0);
    let Some(record) = plan_preview_legs(plan, session.flight_plan.guidance_leg_geometry())
        .into_iter()
...
fn active_guidance_detail_geometry(session: &UiSession) -> Option<(String, GuidanceLegGeometry)> {
    session.flight_plan.active_guidance_detail_geometry()
}
...
fn sequence_guidance_by_ownship_motion(
    ...
    session
        .flight_plan
        .sequence_by_ownship_motion(previous_position, current_position)
}
...
mod tests {
    ...
    fn isolated_test_session(nav_kv_store: Option<NavKvStore>) -> UiSession {
        ...
        let mut nav_data = NavDataController::default();
        if let (Some(store_id), Some(store)) = (nav_kv_store_id, nav_kv_store.as_ref()) {
            nav_data.attach(store_id, store, None);
        }
        ...
        let flight_plan = app_state
            .active_plan
            .map(|plan| FlightPlanController::new(plan, Vec::new()).expect("flight plan"))
            .unwrap_or_default();
        let mut map = MapController::default();
    ...
    fn model_transaction_rolls_back_model_without_disturbing_runtime() {
        ...
        assert_eq!(session.flight_plan.revision(), 0);
        assert!(session.flight_plan.active_plan().is_none());
        assert_eq!(
            session.map.runtime().terrain_source_tile_cache["seed"].as_ptr(),
            terrain_bytes_address
        );
    ...
    fn unrelated_snapshots_reuse_settings_projection_until_settings_change() {
        ...
        assert_eq!(configured.flight_plan_revision, 0);
        assert_eq!(configured.flight_plan_projection_count, 1);
        assert_eq!(configured.nav_data_revision, 0);
        ...
        assert_eq!(unchanged.flight_plan_revision, 0);
        assert_eq!(unchanged.flight_plan_projection_count, 1);
        assert_eq!(unchanged.nav_data_revision, configured.nav_data_revision);
        assert_eq!(
        ...
        assert_eq!(changed.flight_plan_revision, unchanged.flight_plan_revision);
        assert_eq!(changed.nav_data_revision, unchanged.nav_data_revision);
        assert_eq!(
            changed.flight_plan_projection_count,
            unchanged.flight_plan_projection_count
        );
        assert_eq!(
    ...
    #[test]
    fn nav_data_revision_changes_only_when_attached_state_changes() {
        let init =
            create_ui_session(FlightPlan::default(), &[], None, None).expect("create session");
        let initial = session_diagnostics(init.handle).expect("initial diagnostics");
        let store = crate::navkv::nav_kv_store_for_test(&[], 256);

        attach_nav_kv_store_to_session(init.handle, 41, &store).expect("attach NAVDB");
        let attached = session_diagnostics(init.handle).expect("attached diagnostics");
        assert_eq!(attached.nav_data_revision, initial.nav_data_revision + 1);

        get_session_snapshot(init.handle).expect("snapshot attached NAVDB");
        let projected = session_diagnostics(init.handle).expect("projected diagnostics");
        assert_eq!(projected.nav_data_revision, attached.nav_data_revision);

        destroy_session(init.handle);
    }

    #[test]
    fn flight_plan_projection_rebuilds_only_after_flight_plan_changes() {
        let init =
            create_ui_session(FlightPlan::default(), &[], None, None).expect("create session");
        let initial = session_diagnostics(init.handle).expect("initial diagnostics");
        ...
        perform_altitude_planner_action_in_session(
            init.handle,
            "altitude-planner:select:16000".to_string(),
        )
        .expect("change cruise altitude");
        let changed = session_diagnostics(init.handle).expect("changed diagnostics");
        assert_eq!(
            changed.flight_plan_revision,
            initial.flight_plan_revision + 1
        );
        assert_eq!(
            changed.flight_plan_projection_count,
            initial.flight_plan_projection_count + 1
        );
        ...
        assert_eq!(changed.situation_revision, initial.situation_revision);

        get_session_snapshot(init.handle).expect("repeat snapshot");
        let repeated = session_diagnostics(init.handle).expect("repeated diagnostics");
        assert_eq!(repeated.flight_plan_revision, changed.flight_plan_revision);
        assert_eq!(
            repeated.flight_plan_projection_count,
            changed.flight_plan_projection_count
        );
        destroy_session(init.handle);
    }
    ...
    fn nav_data_invalidation_is_generation_scoped_and_centralized() {
        ...
        let store = crate::navkv::nav_kv_store_for_test(&[], 256);
        nav_data.attach(1, &store, None);
        map.invalidate_nav_data();
        weather.invalidate_nav_data();

        assert_eq!(nav_data.generation(), 1);
        assert!(map.runtime().terrain_source_tile_cache.is_empty());
        assert!(map.runtime().agl_terrain_resource_ids_in_flight.is_empty());
    ...
    fn query_obstacle_feature_ids(handle: u32, metrics: &MapSurfaceMetrics) -> Vec<String> {
        let (config, obstacle_cache) = {
            ...
            (
                session.map.overlay_config().clone(),
    ...
    fn nav_db_maintenance_requests_advance_when_installed_next_cycle_becomes_effective() {
        ...
        assert_eq!(just_after.action, NavDbMaintenanceAction::AttemptAdvance);
        assert_eq!(
            just_after
                .snapshot
                .active_nav_db
    ...
    fn nav_db_advance_commits_candidate_atomically_and_advances_epoch_once() {
        ...
        assert_eq!(session.nav_data.epoch(), 1);
        assert_eq!(session.nav_data.store_id(), Some(2));
        assert_eq!(
            session
                .nav_data
                .active_artifact()
    ...
    fn nav_db_advance_page_fault_does_not_mutate_live_generation() {
        ...
        let (live_generation_before, live_revision_before) = {
            let slot = session_slot(init.handle).expect("session slot");
            let session = slot.lock_running().expect("live session");
            (session.nav_data.generation(), session.nav_data.revision())
        };
        let entries = [
            ("chart/catalog", br#"[]"#.as_slice()),
            ("vector/manifest", minimal_vector_manifest_json().as_bytes()),
        ...
        assert!(matches!(first, HadOperationOutcome::NeedResources { .. }));
        {
            let slot = session_slot(init.handle).expect("session slot");
            let session = slot.lock_running().expect("live session");
            assert_eq!(session.nav_data.epoch(), 0);
            assert_eq!(session.nav_data.store_id(), Some(11));
            assert_eq!(session.nav_data.generation(), live_generation_before);
            assert_eq!(session.nav_data.revision(), live_revision_before);
            assert_eq!(
                session
                    .nav_data
                    .active_artifact()
    ...
    fn nav_db_advance_rejects_bad_candidate_and_pins_old_artifact() {
        ...
        let slot = session_slot(init.handle).expect("session slot");
        let session = slot.lock_running().expect("live session");
        assert_eq!(session.nav_data.store_id(), Some(21));
        assert!(session.nav_data.advance_blocked());
        drop(session);
    ...
    fn nav_db_advance_rejects_candidate_missing_required_plan_waypoint() {
        ...
        assert_eq!(session.nav_data.store_id(), Some(31));
    ...
    fn detail_catalog_is_always_available_on_web_and_requires_install_on_android() {
        ...
        let web = raster_catalog_for_layer_state(
            &catalog,
            MapController::default().layer_state(),
            CoreResourcePolicy::PublicUnpacked,
            &BTreeSet::new(),
        );
    ...
    fn unresolved_metar_station_importance_does_not_block_vector_overlay() {
        ...
        attach_isolated_test_nav_kv_store(init.handle, &store);
        {
            ...
            session.map.runtime_mut().vector_tile_cache.insert(
    ...
    fn live_obstacle_current_change_drops_stale_had_before_new_pages_arrive() {
...

        {
            ...
            assert!(session.map.overlay_config().obstacle_layer.is_none());
        }
        let feature_ids = query_obstacle_feature_ids(init.handle, &metrics);
        assert!(!feature_ids.iter().any(|id| id == "obstacle:old"));
    ...
    fn jit_winds_aloft_resolves_selected_snapshot_dependencies_without_background_effects() {
...

        {
            ...
            session
                .flight_plan
                .install_guidance_leg_geometry(vec![GuidanceLegGeometry {
    ...
    fn expired_displayed_chart_records_warning() {
        let init = create_current_test_session();
        {
            ...
            session
                .map
                .replace_raster_catalog(Some(expired_raster_catalog("2020-01-01")));
        }
    ...
    fn expired_background_chart_records_warning_when_selected_chart_is_current() {
        ...
        let tac = raster_map_option("tac:nw", "NW TAC", Some("2026-05-14"), Some("2026-07-09"));
        {
            ...
            session
                .map
                .replace_raster_catalog(Some(raster_catalog_with_displayed_maps(
                    tac.clone(),
                    vec![sectional, tac],
                )));
        }
...
{"data":{"kind":"sample","sample":{"event_time_epoch_ms":300000,"received_time_epoch_ms":300000,"source_kind":"device_gps","position":{"lat":47.350,"lon":-121.410},"altitude_msl_ft":3100.0,"ground_speed_kt":40.0,"track_deg_true":95.0}},"logged_at_epoch_ms":300000,"tag":"ownship.gps_capture.sample"}"#,
    ...
    fn manual_sequence_display_endpoint_is_not_a_sequencing_finish_line() {
        ...
        assert!(
            crate::flight_plan_controller::active_detail_finish_criterion_for_test(
                &plan,
                1,
                &geometry_by_id,
                false,
